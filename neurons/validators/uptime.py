...
class MinerUptimeManager:
    ...
    def up(self, uid, hotkey):
        try:
            ...
            logger.error('Error occurred during uptime end', miner_hotkey=f"{hotkey}", error=f"{traceback.format_exc()}")
    ...
    def down(self, uid, hotkey):
        try:
            ...
            logger.error('Error occurred during downtime start', miner_hotkey=f"{hotkey}", error=f"{traceback.format_exc()}")
    ...
    def get_miner(self, hotkey):
        try:
            ...
            logger.error('Error occurred during miner retrieval', miner_hotkey=f"{hotkey}", error=f"{traceback.format_exc()}")
            return None
    ...
    def calculate_uptimes(self, hotkey, period_seconds):
        try:
            ...
            logger.error('Error occurred during uptime calculation', miner_hotkey=f"{hotkey}", error=f"{traceback.format_exc()}")
            raise e
