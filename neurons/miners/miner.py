...
class Miner(BaseMinerNeuron):
    ...
    async def block_check(self, synapse: protocol.BlockCheck) -> protocol.BlockCheck:
        try:
            ...
            logger.error('Error', error=f"{traceback.format_exc()}")
            synapse.output = None
        return synapse
    ...
    async def benchmark(self, synapse: protocol.Benchmark) -> protocol.Benchmark:
        try:
            ...
            if match is None:
                ...
                logger.error('Invalid benchmark query', query=f"{synapse.query}")
                synapse.output = None
            else:
    ...
    def base_priority(self, synapse: bt.Synapse) -> float:
        ...
        logger.trace("Prioritizing hotkey with value", hotkey=f"{synapse.dendrite.hotkey}", value=f"{prirority}")
        return prirority
    ...
    def set_weights(self):
        ...
        try:
            ...
            logger.error('Failed to set weights on chain with exception', error=f"{e}")

        bt.logging.info(f"Set weights: {chain_weights}")
        logger.info('set weights', weights=f"{chain_weights}")
