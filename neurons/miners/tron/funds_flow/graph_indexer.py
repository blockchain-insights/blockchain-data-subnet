...
logger = setup_logger("TronGraphIndexer")
            ...
            for index_name, statement in index_creation_statements.items():
                if index_name not in existing_index_set:
                    try:
                        ...
                        logu.logger.error("An exception occurred while creating index", index_name=f"{index_name}", error=f"{e}")
            ...
            try:
                for i in range(0, len(transactions), batch_size):
                    ...
                    transaction.run(
                        ...
                        MERGE (from:Address {address: tx.from_address})
                        MERGE (to:Address {address: tx.to_address})
                        CREATE (from)-[:SENT { tx_hash: tx.tx_hash, block_number:tx.block_number, value: tx.value, timestamp: tx.timestamp, symbol:tx.symbol, from: tx.from_address, to: tx.to_address, checksum:tx.checksum }]->(to)
                        """,
                        transactions = [
                            {
                                "tx_hash": tx.tx_hash,
                ...
                logu.logger.error(f"An exception occurred", error=f"{e}")
                return False
