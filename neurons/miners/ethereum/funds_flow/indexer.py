...
def single_index(_graph_creator, _graph_indexer, _graph_search, start_height: int, end_height: int, is_reverse_order: bool = False):
        ...
        try:
            ...
            if tx['cacheCnt'] > CACHE_COUNT or (last - start) * direction == 0:
                ...
                if success:
                    ...
                    logger.info(f"[Main Thread]", extra = logger_extra_data(finished_block = start, tps = formatted_tps, spent_time = time.time() - start_time))
            ...
            if shutdown_flag:
                logger.info(f"Finished indexing block before shutdown.", extra = logger_extra_data(block = start))
                break
    ...
    logger.info(f"Finished Single Main Indexing", extra = logger_extra_data(start_block = start_height, end_block = end_height))
...
def index_blocks(_graph_creator, _graph_indexer, _graph_search, start_height):
    ...
    while not shutdown_flag:
        ...
        if current_block_height - skip_blocks < 0:
            logger.info(f"Waiting for blocks to be mined.", extra = logger_extra_data(skip_blocks = skip_blocks))
            time.sleep(3)
            continue
        ...
        if block_height > current_block_height:
            if tx['cacheCnt'] > CACHE_COUNT:
                ...
                if success:
                    time_taken = time.time() - buf_time
                    formatted_tps = tx['cacheCnt'] / time_taken if time_taken > 0 else float("inf")
                    logger.info(f"[Main Thread]", extra = logger_extra_data(tps = formatted_tps, spent_time = time.time() - start_time))
                else:
                    # sometimes we may have memgraph server connect issue catch all failed block so we can retry
        ...
        while block_height <= current_block_height - skip_blocks:
            try:
                ...
                if shutdown_flag:
                    logger.info(f"Finished indexing block before shutdown.", extra = logger_extra_data(block_height = block_height))
                    break
            except Exception as e:
                # sometimes we may have rpc server connect issue catch all failed block so we can retry
        ...
        try:
            ...
            if inprogress_flag:
                if not log_display:
                    ...
                    logger.info(f"[Sub Thread]", extra = logger_extra_data(thread_index = index, start_block = start, last_block = last, completed_percent = formatted_percent))
                    log_display = True
                time.sleep(0.01)
                continue
            else:
            ...
            if shutdown_flag:
                logger.info(f"Finished indexing block before shutdown.", extra = logger_extra_data(block = block_height))
                break
        except Exception as e:
            # sometimes we may have rpc server connect issue catch all failed block so we can retry
            log_blockHeight_crashed_by_rpc(block_height)
    ...
    try:
        ...
        # Only Main Indexer running
            ...
            if main_start_height > 0 and main_last_height > 0:
                logger.info(f'-- Main thread is running for indexing based on range of block numbers --', extra = logger_extra_data(min_block = main_start_height, max_block = main_last_height))
        ...
        logger.error(f"Retry failed with error", extra = logger_extra_data(error = {'exception_type': e.__class__.__name__,'exception_message': str(e),'exception_args': e.args}))
        logger.info(f"Retrying in seconds...", extra = logger_extra_data(retry_delay = retry_delay))
