...
def query_blacklist(self, synapse: protocol.Query) -> typing.Tuple[bool, str]:
        ...
        if  self.config.network != synapse.network:
            bt.logging.trace("Blacklisting hot key because of wrong blockchain", hotkey = hotkey)
            return True, "Network not supported."
        if self.config.model_type != synapse.model_type:
            bt.logging.trace("Blacklisting hot key because of wrong model type", hotkey = hotkey)
            return True, "Model type not supported."
        if not is_query_only(self.miner_config.query_restricted_keywords, synapse.query):
            bt.logging.trace("Blacklisting hot key because of illegal cypher keywords", hotkey = hotkey)
...
def discovery_blacklist(self, synapse: protocol.Discovery) -> typing.Tuple[bool, str]:
    ...
    bt.logging.debug("Stake of hotkey", hotkey = hotkey, stake = stake)
...
def base_blacklist(self, synapse: bt.Synapse) -> typing.Tuple[bool, str]:
    ...
    if hotkey not in self.metagraph.hotkeys:
        bt.logging.trace(f"Blacklisting unrecognized hotkey", hotkey = hotkey)
